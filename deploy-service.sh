...
if [ "${ROLLING}" = "rolling" ]; then
  ...
  kubectl rolling-update ${SERVICENAME} --image=${DOCKER_REGISTRY}/${CONTAINER1}:latest || true
  
else

  # delete service (throws and error to ignore if service does not exist already)
  for f in ${DEPLOYDIR}/*.yaml; do envsubst < $f > kubetemp.yaml; cat kubetemp.yaml; echo ""; kubectl delete --namespace=${kubenamespace} -f kubetemp.yaml || true; done

  # create service (does nothing if the service already exists)
  for f in ${DEPLOYDIR}/*.yaml; do envsubst < $f > kubetemp.yaml; kubectl create --namespace=${kubenamespace} -f kubetemp.yaml --validate=false || true; done
fi
